...
def stagn_train_2d(
    ...
    for epoch in range(epochs):
        ...
        pred = np.array(pred)
        print(
            f"Epoch: {epoch}, loss: {loss:.4f}, auc: {roc_auc_score(true, pred):.4f}, F1: {f1_score(true, pred, average='macro'):.4f}, AP: {average_precision_score(true, pred):.4f}"
        )
...
def load_stagn_data(args: dict):
    ...
    loc_feature = np.array(
        loc_enc.fit_transform(
            sampled_df['Location'].to_numpy()[:, np.newaxis]
        ).todense()
    )
    ...
    g.add_edges(
        encoded_source,
        encoded_tgt,
        data={'feat': torch.from_numpy(loc_feature).to(torch.float32)},
    )
