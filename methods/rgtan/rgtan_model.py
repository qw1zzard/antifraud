    ...
    def __init__(
        ...
        if isinstance(neigh_features, dict):
            self.nei_table = Tabular1DCNN2(
                input_dim=len(neigh_features), embed_dim=in_feats_dim
            )
...
class RGTAN(nn.Module):
    def __init__(
        ...
        if n2v_feat:
            ...
            self.nei_feat_dim = (
                len(neigh_features.keys()) if isinstance(neigh_features, dict) else 0
            )
        else:
        ...
        self.layers.append(
            nn.Sequential(
                ...
                nn.Linear(
                    self.hidden_dim * self.heads[0], in_feats + self.nei_feat_dim
                ),
            )
        )
        ...
        self.layers.append(
            TransformerConv(
                in_feats=self.in_feats + self.nei_feat_dim,
    ...
    def forward(self, blocks, features, labels, n2v_feat=None, neighstat_feat=None):
        ...
        label_embed = self.layers[1](h) + self.layers[2](label_embed)  # 2926, 2926, 256
