...
class stan_model(nn.Module):
    ...
    def __init__(
        ...
        # attention layer
        self.attention_W_1 = nn.Parameter(
            torch.Tensor(self.feat_dim, self.attention_hidden_dim).uniform_(0.0, 1.0)
        )
        self.attention_W_2 = nn.Parameter(
            torch.Tensor(self.feat_dim * 2, self.attention_hidden_dim).uniform_(
                0.0, 1.0
            )
        )
        self.attention_U_1 = nn.Parameter(
            torch.Tensor(self.feat_dim, self.attention_hidden_dim).uniform_(0.0, 1.0)
        )
        self.attention_U_2 = nn.Parameter(
            torch.Tensor(self.feat_dim * 2, self.attention_hidden_dim).uniform_(
                0.0, 1.0
            )
        )
        self.attention_V_1 = nn.Parameter(
            torch.Tensor(self.attention_hidden_dim, 1).uniform_(0.0, 1.0)
        )
        self.attention_V_2 = nn.Parameter(
            torch.Tensor(self.attention_hidden_dim, 1).uniform_(0.0, 1.0)
        )
    ...
    def attention_layer(
        ...
        input_conv = torch.reshape(
            torch.concat(self.output_att, axis=dim),
            [
                -1,
                self.time_windows_dim,
                self.spatio_windows_dim,
                self.feat_dim * 2 * dim,
            ],
        )
        ...
        for i in range(len(x)):
            ...
            if dim == 1:
                att_hidden = torch.tanh(
                    torch.add(
                        torch.matmul(x_i, self.attention_W_1),
                        torch.matmul(output, self.attention_U_1),
                    )
                )
                ...
                att_hidden = torch.tanh(
                    torch.add(
                        torch.matmul(x_i, self.attention_W_2),
                        torch.matmul(output, self.attention_U_2),
                    )
                )
        ...
        for j, (alpha_i_j, output) in enumerate(zip(alpha_i, x)):
            if j == index:
                ...
                output = output.reshape(
                    -1, self.windows_dim[2 - dim], self.feat_dim * dim
                )
